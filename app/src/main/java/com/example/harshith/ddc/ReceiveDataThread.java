...
public class ReceiveDataThread extends Thread {
    ...
    public ReceiveDataThread(BluetoothSocket bluetoothSocket, Handler handler,GlobalClass globalClass) {
        ...
        q=3; hand = new int [] {1,0,0,1,8,8,8,8,1,8};
        // call
        a[q].updateFrame(hand);

        q=4; hand = new int [] {0,0,1,0,8,8,8,-1,8,8};
        // cam open
        a[q].updateFrame(hand);
                ...
                if (endOfLineIndex > 0) {
                    ...
                    if(count == no){
                        ...
                        if(gestActive == 0){
                            ...
                            Looper.prepare();
                            handler.obtainMessage(Constants.READ_STATUS,Constants.CAMERA_CLICK).sendToTarget();
                            Looper.loop();
                        }
                        else if(gestActive == 5){
                            Looper.prepare();
                            handler.obtainMessage(Constants.READ_STATUS,Constants.VOLUME_UP).sendToTarget();
                            Looper.loop();
                        }
                        else if(gestActive == 6){
                            Looper.prepare();
                            handler.obtainMessage(Constants.READ_STATUS,Constants.VOLUME_DOWN).sendToTarget();
                            Looper.loop();
                        }
                        else if(gestActive == 7){
                            Looper.prepare();
                            handler.obtainMessage(Constants.READ_STATUS,Constants.END_CALL).sendToTarget();
                            Looper.loop();
                        }
                        count = 0;
                        gestActive = -1;
